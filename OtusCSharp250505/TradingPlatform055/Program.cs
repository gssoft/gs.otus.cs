...
// 3. Регистрируем InMemoryLogDatabase (новый сервис для логов трейдинга)
...
// 4. Регистрируем менеджера тикеров
builder.Services.AddSingleton<EventHubTickerManager>();

// 5. Регистрируем ChartContainer КАК ФАБРИКУ, чтобы контролировать время создания
...
// 6. Регистрируем InMemoryTradingDatabase ДО фоновых сервисов
...
// 7. SignalR ДО фоновых сервисов
builder.Services.AddSignalR();

// 8. Регистрируем фоновые сервисы в правильном порядке
builder.Services.AddHostedService<QuotesGeneratorService>();
builder.Services.AddHostedService<StrategiesExecutionService>();

// QuotesConsoleService должен получать IInMemoryLogDatabase
builder.Services.AddHostedService<QuotesConsoleService>();

// TradingMonitorService должен получать IInMemoryLogDatabase
...
// InMemoryLogDatabase как hosted service
builder.Services.AddHostedService(provider =>
    provider.GetRequiredService<InMemoryLogDatabase>());

// 9. Регистрация обработчиков событий
...
{
    var logs = logDatabase.GetPagedLogs(ticker, strategy, level, category, page, 50);

    return Results.Ok(new
    {
        Items = logs.Items.Select(l => new
        {
            l.Id,
            l.Timestamp,
            l.Ticker,
...
// API для статистики логов
app.MapGet("/api/trading/logs/stats", (IInMemoryLogDatabase logDatabase) =>
{
    var logs = logDatabase.GetLogs();

    return Results.Ok(new
    {
        TotalCount = logDatabase.GetTotalCount(),
        ByLevel = logs
            .GroupBy(l => l.Level)
            .Select(g => new
            {
        ...
        ByCategory = logs
            ...
            .OrderByDescending(g => g.Count),
        ByTicker = logs
            .Where(l => !string.IsNullOrEmpty(l.Ticker))
            .GroupBy(l => l.Ticker)
            .Select(g => new
            {
                Ticker = g.Key,
    ...
    IInMemoryLogDatabase logDatabase) =>
{
    ...
    return Results.Ok(new
    {
        ...
        {
            Status = "Active",
            LogsCount = logDatabase.GetTotalCount(),
...
app.Lifetime.ApplicationStarted.Register(() =>
{
            ...
            if (addresses != null && addresses.Any())
            {
                ...
                if (displayConfig.ShowApiEndpoints)
                {
                    ...
                    var apiEndpoints = new[]
                    {
                        ...
                        new { Name = "Логи трейдинга", Path = "/api/trading/logs" },
                        new { Name = "Статистика логов", Path = "/api/trading/logs/stats" }
                    };
    ...
    Console.WriteLine($"✅ InMemoryLogDatabase: {logDatabase.GetTotalCount()} записей (24 часа)");
    ...
    if (fileConfig.EnableFileOutput)
    {
        try
        {
            ...
            var statsContent = $"СТАТИСТИКА СИСТЕМЫ - {appInfoConfig.Name} v{appInfoConfig.Version}\n" +
                              ...
                              $"Логов трейдинга: {logDatabase.GetTotalCount()}\n" +
                              $"\n📈 ТИКЕРЫ И СТРАТЕГИИ:\n";
    ...
    logDatabase.AddLog($"🚀 {appInfoConfig.Name} v{appInfoConfig.Version} успешно запущена");
});
...
app.Lifetime.ApplicationStopping.Register(() =>
{
    ...
    try
    {
        ...
        var logDatabase = app.Services.GetRequiredService<IInMemoryLogDatabase>();
        logDatabase.AddLog("🛑 Приложение остановлено");
    }
    catch (Exception ex)
    {
...
//// Добавим в Program.cs после других API endpoints
//app.MapGet("/api/trading/logs", (IInMemoryLogDatabase logDatabase,
//    [FromQuery] string? ticker,
//    [FromQuery] string? strategy,
//    [FromQuery] string? level,
